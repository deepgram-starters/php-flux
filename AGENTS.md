...
PHP demo app for Deepgram Flux.

## Architecture

- **Backend:** PHP (PHP) on port 8081
...
| File | Purpose |
|------|---------|
| `server.php` | Main backend — API endpoints and WebSocket proxy |
...
- **Backend:** `composer.json` — Uses Composer for package management. PHP built-in server or ReactPHP for WebSocket.
...
4. Handle it in the backend `server.php` — read the param and pass it to the Deepgram API
...
```
feat(php-flux): add diarization support
fix(php-flux): resolve WebSocket close handling
refactor(php-flux): simplify session endpoint
